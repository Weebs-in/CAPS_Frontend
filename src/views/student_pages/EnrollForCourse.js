...
const EnrollForCourse = () => {
    ...
    const fetchCourses = async () => {
        ...
        await fetch(config.getAllCoursesStudentCanEnroll + `?${params.toString()}`, {
            headers: {
    ...
    const handleEnrollDetails = async (courseId) => {
        ...
        if (selectedCourse.courseEnrollmentStatus === "Not Enrolling") {
            addToast(resultToast({
                toastHeader: "Not Enrolling",
                toastColor: config.TOAST_FAILED_COLOR,
                toastMessage: "failed, because course is currently not enrolling."
            }));
        } else {
    ...
    const handleEnroll = async (event) => {
        ...
        try {
            ...
            // TODO: FIX THIS
            params.append('studentId', 1);
            const response = await fetch(config.studentEnrollCourse + `?${params.toString()}`, {
                method: 'POST'
            });
            if (response.ok) {
        ...
        <CRow>
            <CCol xs={12}>
                <CCard className="mb-4">
                    <CCardHeader>
                        <h3 style={{marginTop: 10 + 'px'}}>Enroll For A Course</h3>
                    </CCardHeader>
                    <CCardBody>
